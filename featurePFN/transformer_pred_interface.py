...
""" 
Modified version of the official TabPFN code, allows the classifier to produce gradients for backpropagation.
...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, overwrite_warning=False):
        ...
        check_X_y(X.detach().cpu().numpy(), y.cpu().numpy(), force_all_finite=False)
        # Store the classes seen during fit
        self._validate_targets(y.cpu().numpy())
    ...
    def predict_proba(self, X, normalize_with_test=False):
        ...
        X_full = torch.cat([self.X_, X], axis=0).unsqueeze(1)
        y_full = torch.cat([self.y_, torch.zeros_like(X[:, 0])], axis=0).unsqueeze(1)
...
#                return 'robust_all'
    ...
    for ensemble_configuration in ensemble_configurations:
        ...
        eval_xs_, eval_ys_ = eval_xs, eval_ys.clone()
